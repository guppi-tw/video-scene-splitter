...
- 複数動画の一括シーン検出（「一括検出」）。進捗画面は処理を続けたまま隠せて、キューの「進捗を表示」から戻せます。一括検出した動画を開くと日付検出だけをバックグラウンドで実行し、補正モーダルは自動表示しません
- キューの状態フィルター（待機中 / 確認が必要 / 書き出し可能 / エラー / 完了）
- 短いクリップ、日付未設定・推定、長い無音、フェード候補を理由付きでまとめる確認事項リスト。「次を確認」で未確認クリップだけを順番に確認できます
...
- PySceneDetect によるシーン境界の自動検出候補追加
  ...
  - 検出中の進捗表示と途中中止
- 自動検出後は焼き込み日付だけをバックグラウンドで検出。つなぎ目除外と短いシーンの結合は、必要なときだけ「補正ツール」から実行
...
- クリップごとの「書き出す」切り替え
- クラウド共有前に確認したいクリップの「要確認」マーキングと専用フォルダへの書き出し
...
4. 自動検出後は焼き込み日付をバックグラウンドで検出します（macOS）。つなぎ目除外、短いシーンの結合、音声・フェード解析が必要な場合だけ、右側の「補正ツール」から選択します。音声・フェードの候補は「その他」メニューから境界へ追加するまで動画を変更しません。
5. 「確認事項」が表示されたら「次を確認」で該当クリップへ移動し、内容を確認して「確認済み」にします。境界が曖昧な場合は「境界確認」で前後フレームを並べて調整できます。
6. 右側のクリップ一覧で書き出し対象、イベント名、日付、ファイル名を調整します。ファイル名は表示をダブルクリックするか F2 で編集できます。「クリップを結合」を選ぶと連続シーンをまとめられます。複数動画へ同じ情報を付ける場合は、キューの「複数動画をまとめて設定」を使います。
7. 子供のプールなどクラウド共有前に確認したいクリップは「要確認」にします。書き出し時に専用フォルダへ分けられます。
