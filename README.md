...
- PySceneDetect によるシーン境界の検出・分割点追加
  ...
  - 検出中の進捗表示と途中中止
- タイムラインの「シーンを検出して分割」と「単色区間をトリミング」から、通常のカット検出と単色区間の除外を目的別に実行
...
3. プレビューの「分割 [S]」ボタン、タイムライン操作、またはタイムラインの「シーンを検出して分割」で分割位置を追加します。シーン検出では、感度と最小シーン長を変えて再検出し、青い仮境界・反映後のクリップ数・境界前後の画像を確認してから「候補を反映」します。
4. 黒・白・青などの単色区間を除外したい場合は、タイムラインの「単色区間をトリミング」を実行し、検出結果を確認して反映します。短いシーンの結合、音声・フェード解析などは、必要な場合だけ右側の「補正ツール」から選択します。音声・フェードの候補はタイムラインの「候補 N件を追加」を押すまで動画を変更しません。
...
| シーンを検出して分割 | 感度と最小シーン長を調整し、未確定の分割候補をプレビュー |
| 単色区間をトリミング | 黒・白・青などの単色区間を検出し、確認後に書き出し対象から除外 |
