...
- 複数動画の一括シーン検出（「一括シーン検出」）。進捗画面は処理を続けたまま隠せて、キューの「進捗を表示」から戻せます。一括シーン検出した動画を開くと日付検出だけをバックグラウンドで実行し、補正モーダルは自動表示しません
...
3. プレビューの「分割 [S]」ボタン、タイムライン操作、またはタイムラインの「シーン検出」で分割位置を追加します。感度と最小シーン長は「…」内の「シーン検出設定」から調整できます。
